<template>
  <div class="ocr-log">
    <div class="content-card">
      <div class="toolbar">
        ...
        <select v-model="filterStatus" class="filter-select" @change="handleSearch">
          ...
          <option value="RECOGNIZED">成功</option>
        ...
        <span class="toolbar-stat">
          共 <strong>{{ adminStore.ocrLogs?.total ?? 0 }}</strong> 条
        </span>
      </div>
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th style="min-width:200px">输入内容</th>
              <th style="min-width:200px">输出结果</th>
          ...
          <tbody>
            <tr v-for="(row, index) in adminStore.ocrLogs?.items ?? []" :key="row.id">
              ...
              <td>
                <span class="content-text" :title="row.inputContent">
                  {{ truncateText(row.inputContent, 50) }}
                </span>
              </td>
              <td>
                <span v-if="row.outputResult" class="content-text" :title="row.outputResult">
                  {{ truncateText(row.outputResult, 50) }}
                </span>
              ...
              <td>
                <span v-if="row.processTime" class="time-text">{{ row.processTime }}ms</span>
                <span v-else class="na-text">-</span>
              </td>
            ...
            <tr v-if="!adminStore.loading && (!adminStore.ocrLogs?.items?.length)">
      ...
      <div class="footer" v-if="adminStore.ocrLogs?.total">
        <span class="footer-info">
          第 {{ (currentPage - 1) * pageSize + 1 }}-{{ Math.min(currentPage * pageSize, adminStore.ocrLogs?.total ?? 0) }} 条，共 {{ adminStore.ocrLogs?.total ?? 0 }} 条
        </span>
        <div class="pager">
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="OCR日志详情" width="600px" destroy-on-close>
      <div v-if="currentLog" class="detail-content">
        ...
        <div class="detail-row">
          <label>处理时间：</label>
          <span>{{ currentLog.processTime ? currentLog.processTime + 'ms' : '-' }}</span>
        </div>
        <div class="detail-row">
          <label>创建时间：</label>
        ...
        <div class="detail-row detail-row-full">
          <label>输入内容：</label>
          <div class="detail-box">{{ currentLog.inputContent }}</div>
        </div>
        <div class="detail-row detail-row-full">
          <label>输出结果：</label>
          <div class="detail-box">{{ currentLog.outputResult || '-' }}</div>
        </div>
      </div>
    </el-dialog>
  </div>
...
import type { OcrLog } from '@/types/admin'
...
const currentLog = ref<OcrLog | null>(null)

const totalPages = computed(() => Math.ceil((adminStore.ocrLogs?.total ?? 0) / pageSize))
...
const handleViewDetail = (row: OcrLog) => {
  currentLog.value = row
  detailVisible.value = true
}

const handleDelete = async (row: OcrLog) => {
  try {
    await ElMessageBox.confirm(`确定要删除该OCR日志吗？`, '确认删除', {
      confirmButtonText: '删除',
      cancelButtonText: '取消',
      type: 'warning'
    })
    await adminStore.deleteOcrLog(row.id)
