<template>
  <div class="export-log">
    <div class="content-card">
      <div class="toolbar">
        ...
        <span class="toolbar-stat">
          共 <strong>{{ adminStore.exportLogs?.total ?? 0 }}</strong> 条
        </span>
      </div>
        ...
        <table class="table">
          <thead>
            <tr>
              ...
              <th style="min-width:140px">模板名称</th>
              <th style="min-width:160px">文件名</th>
              <th style="width:90px">文件大小</th>
              <th style="width:80px">字符数</th>
              <th style="width:80px">状态</th>
              <th style="width:80px">耗时</th>
              <th style="min-width:120px">错误信息</th>
              <th style="width:170px">创建时间</th>
              <th style="width:140px;text-align:center;position:sticky;right:0;background:var(--bg-header)">操作</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(row, index) in adminStore.exportLogs?.items ?? []" :key="row.id">
              ...
              <td><span class="template-name">{{ row.templateName || '-' }}</span></td>
              <td><span class="file-name" :title="row.fileName">{{ truncateText(row.fileName, 20) }}</span></td>
              <td>
                <span v-if="row.fileSize" class="size-text">{{ formatFileSize(row.fileSize) }}</span>
                <span v-else class="na-text">-</span>
              </td>
              <td>
                <span v-if="row.charCount" class="char-text">{{ row.charCount }}</span>
                <span v-else class="na-text">-</span>
              </td>
              <td>
              ...
              <td>
                <span v-if="row.duration" class="time-text">{{ row.duration }}ms</span>
                <span v-else class="na-text">-</span>
              </td>
              <td>
                <span v-if="row.errorMessage" class="error-text" :title="row.errorMessage">
                  {{ truncateText(row.errorMessage, 15) }}
                </span>
                <span v-else class="na-text">-</span>
              </td>
            ...
            <tr v-if="!adminStore.loading && (!adminStore.exportLogs?.items?.length)">
      ...
      <div class="footer" v-if="adminStore.exportLogs?.total">
        <span class="footer-info">
          第 {{ (currentPage - 1) * pageSize + 1 }}-{{ Math.min(currentPage * pageSize, adminStore.exportLogs?.total ?? 0) }} 条，共 {{ adminStore.exportLogs?.total ?? 0 }} 条
        </span>
        <div class="pager">
    ...
    <!-- 详情弹窗 -->
    <el-dialog v-model="detailVisible" title="导出日志详情" width="600px" destroy-on-close>
      <div v-if="currentLog" class="detail-content">
        ...
        <div class="detail-row">
          <label>模板名称：</label>
          <span>{{ currentLog.templateName || '-' }}</span>
        </div>
        <div class="detail-row">
          <label>文件名：</label>
          <span>{{ currentLog.fileName || '-' }}</span>
        </div>
        <div class="detail-row">
          <label>文件大小：</label>
          <span>{{ currentLog.fileSize ? formatFileSize(currentLog.fileSize) : '-' }}</span>
        </div>
        <div class="detail-row">
          <label>字符数：</label>
          <span>{{ currentLog.charCount || '-' }}</span>
        </div>
        <div class="detail-row">
          <label>状态：</label>
        ...
        <div class="detail-row">
          <label>耗时：</label>
          <span>{{ currentLog.duration ? currentLog.duration + 'ms' : '-' }}</span>
        </div>
        <div class="detail-row">
          <label>创建时间：</label>
          <span>{{ formatTime(currentLog.createdAt) }}</span>
        </div>
        <div v-if="currentLog.errorMessage" class="detail-row detail-row-full">
          <label>错误信息：</label>
          <div class="error-box">{{ currentLog.errorMessage }}</div>
        </div>
      </div>
    </el-dialog>
  </div>
...
const currentLog = ref<ExportLog | null>(null)

const totalPages = computed(() => Math.ceil((adminStore.exportLogs?.total ?? 0) / pageSize))
...
const handleViewDetail = (row: ExportLog) => {
  currentLog.value = row
  detailVisible.value = true
}

const handleDelete = async (row: ExportLog) => {
  try {
    await ElMessageBox.confirm(`确定要删除该导出日志吗？`, '确认删除', {
      confirmButtonText: '删除',
      cancelButtonText: '取消',
      type: 'warning'
    })
    await adminStore.deleteExportLog(row.id)
...
.template-name { font-size: 13px; color: var(--text-primary); }
.file-name { font-size: 13px; color: var(--text-secondary); }
.char-text { font-size: 13px; color: var(--text-secondary); }
.error-text { font-size: 13px; color: var(--c-red); }
...
.size-text { font-size: 13px; color: var(--text-secondary); }
